...
-- BaseLua 'BaseClass' class.
-- @return The valued returned by the 'extend' method of the BaseLua
-- 'BaseClass' class, usually expected to be a new class that subclasses
-- 'BaseClass'.
...
--- Invokes the 'extend' method of the BaseLua 'BaseClass' class, effectively
-- allowing a BaseLua subclass to be created.  Use this method if you are
-- passing in, as the first parameter, a table to be converted into a class.
-- The table should contain a property 'class.name' that defines the name of
-- the resulting class.  If you want to create a new class from scratch, and
-- would rather pass in the class name as a parameter, please see function
-- 'autoclass'.
-- @name class
-- @param ... Optional parameters to be passed to the 'extend' method of the
-- BaseLua 'BaseClass' class.
...
function CLASS.class( ... )
    return require( BaseLua.package.BaseClass ):extend( ... )
end
...
--   "Classes.BaseLua.globals" ) -- "globals"
function CLASS.getClassName( packagePath )
    local _, _, name = string.find(
        packagePath, BaseLua.PACKAGE_NAME_PATTERN )

    return name or packagePath
end
...
--  "Classes.BaseLua.globals" ) -- "Classes.BaseLua"
function CLASS.getPackageName( packagePath )
    local _, _, name = string.find(
        packagePath, BaseLua.PACKAGE_PATH_PATTERN )
